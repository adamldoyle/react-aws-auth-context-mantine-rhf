...
describe('ConfirmAccountForm', () => {
  const renderComponent = (onSubmit = jest.fn(), onResendCode = jest.fn()) => {
    return render(
      <ConfirmAccountForm email="testEmail@gmail.com" confirmAccount={onSubmit} resendCode={onResendCode} />,
    );
  };

  it('displays the email', () => {
    const rendered = renderComponent();
    expect(rendered.getByText('testEmail@gmail.com'));
  });
  ...
  it('requires all fields', async () => {
    ...
    const rendered = renderComponent(onSubmit);
    fireEvent.click(rendered.getByRole('button', { name: 'Confirm account' }));
    await waitFor(() => expect(rendered.queryByText('Code is required')).not.toBeNull());
  ...
  it('provides button to resend code', () => {
    ...
    fireEvent.click(rendered.getByRole('button', { name: "Don't have a code? Resend email" }));
